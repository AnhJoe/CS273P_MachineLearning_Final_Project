...
The multilayer perceptron (MLP) experiments established a strong non-spatial baseline using the log-transformed SVI indicators. The best MLP achieved a test performance of **RMSE = 2.00** and **$R^2 = 0.57$**, indicating that county-level vulnerability indicators explain a meaningful but incomplete portion of the variation in FEMA assistance.

However, counties are not independent observations. Neighboring counties often share regional socioeconomic conditions, infrastructure systems, hazard exposure, and policy contexts. Ignoring these spatial relationships may limit predictive performance. A graph-based model can explicitly incorporate this spatial dependence by representing **counties as nodes and geographic adjacency as edges**, allowing the model to combine local tabular features with information from nearby counties. The goal of the Graph Convolutional Network (GCN) stage is therefore to evaluate whether explicit spatial structure improves prediction compared to the tabular MLP baseline. Graph-based learning approaches provide a principled framework for modeling relationships in network-structured data (Hamilton, 2020).
...
## Baseline Configuration from the MLP

To ensure a fair comparison between model classes, the GCN stage preserves the same preprocessing and evaluation setup used in the MLP experiments. This ensures that any performance differences arise from the use of spatial structure rather than changes in the data pipeline.

The main elements carried forward include:

- Log-transformed feature matrix $X_{\log}$
- Standardized input features using the training split scaler
- Standardized log-transformed target variable
- Fixed train / validation / test splits
- Identical evaluation metrics: **RMSE** and **$R^2$**
- Final comparison against the **saved best MLP configuration**

### Baseline GCN Configuration

The baseline graph convolutional network (GCN) uses the following initial hyperparameter configuration prior to the ablation experiments:
...
These parameters provide a stable starting point for evaluating graph-based learning on the county adjacency network. Subsequent ablation experiments systematically vary architectural and regularization parameters to identify the best-performing configuration before final evaluation on the held-out test set.
...
The first ablation varies the number of graph convolution layers (e.g., **1, 2, and 3 layers**). Each additional layer expands the spatial receptive field, allowing counties to incorporate information from increasingly distant neighbors. However, deeper graph models may suffer from **oversmoothing**, where node representations become overly similar (Hamilton, 2020).

### Hidden Dimension

The second ablation varies the hidden dimension (e.g., **16, 32, and 64 units**). This parameter controls the representational capacity of the model and determines how flexibly the GCN can combine county-level features with neighborhood information.

### Regularization

The final ablation focuses on regularization. Consistent with the MLP findings, weight decay is treated as the primary regularization mechanism, while dropout remains modest. Weight decay values such as **$0$, $10^{-5}$, and $10^{-3}$** are tested alongside dropout values such as **$0.0$, $0.1$, and $0.3$** to evaluate whether similar regularization patterns apply to spatial graph models.
...
The MLP baseline uses a deeper fully connected architecture with hidden layers ([128, 64, 32]), resulting in **12,417 trainable parameters**. Training converged relatively quickly, stopping after **46 epochs**, with the best validation performance occurring at epoch 21. On the held-out test set, the MLP achieved a **test RMSE of 2.00** and an **$R^2$ of 0.571**, indicating that the model explains approximately **57% of the variance** in the log-transformed FEMA IHP outcome. These results suggest that nonlinear interactions among the SVI indicators provide meaningful predictive signal for disaster assistance outcomes.

The GCN model, in contrast, incorporates spatial relationships between counties through a two-layer graph convolutional architecture with 128 hidden units and **2,177 trainable parameters**, substantially fewer than the MLP baseline. Despite this architectural efficiency, the GCN produced weaker predictive performance. Training ran for the full **300 epochs**, with the best validation performance occurring late in training at epoch 300. On the test set, the model achieved a **test RMSE of 2.40** and an **$R^2$ of 0.397**, explaining roughly **40% of the variance** in the target variable.
...
Overall, the **baseline MLP outperforms the spatial GCN model across all evaluation metrics**. The lower RMSE and higher $R^2$ indicate that the tabular MLP architecture captures the predictive structure of the SVI indicators more effectively than the spatial model under the current configuration. One possible explanation is that the county-level socioeconomic variables already encode much of the relevant variation in disaster assistance outcomes, leaving limited additional predictive value for spatial smoothing through graph convolutions. Additionally, the relatively sparse spatial signal in FEMA assistance distributions may reduce the benefits of neighborhood aggregation in the graph model.

These findings highlight that while graph neural networks offer a principled mechanism for incorporating spatial structure, they do not automatically outperform well-tuned tabular models. In this case, the MLP baseline provides a stronger predictive benchmark and serves as an important reference point for evaluating the contribution of spatial modeling approaches.
