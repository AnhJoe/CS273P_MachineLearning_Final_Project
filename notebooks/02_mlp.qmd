...
y_log = pd.read_csv("../data/processed/y_log.csv")
...
where the 15 input units correspond to the log-transformed **EP\_*** vulnerability indicators and the output layer predicts the log-transformed FEMA Individual Household Program (IHP) assistance amount. The SVI indicators are widely used to characterize community-level social vulnerability across socioeconomic, demographic, and housing dimensions (Flanagan et al., 2011; Centers for Disease Control and Prevention/Agency for Toxic Substances and Disease Registry [CDC/ATSDR], 2024).

### Training Configuration

The MLP baseline is trained using the following optimization and training settings:
...
Models are compared primarily using root mean squared error (RMSE) on the validation set, with $R^2$ reported as a complementary measure of explained variance. After selecting the best configuration, the final model is evaluated once on the held-out test set to provide an unbiased estimate of generalization performance. These experiments establish a strong tabular benchmark before introducing spatial graph-based models.

## Ablation Plan

A series of controlled ablation experiments are conducted to evaluate sensitivity to preprocessing, architecture, and regularization. Model selection and hyperparameter tuning are performed using the validation split only, while the held-out test split is reserved for the final evaluation. Each ablation varies a single design component while keeping the remaining settings fixed, allowing the effect of individual modeling choices to be isolated and interpreted.

### Feature Representations

Two feature representations are evaluated: a standardized raw indicator set and a standardized log-transformed indicator set (for a subset of variables identified during the exploratory data analysis). Many socioeconomic indicators exhibit strong right-skewed distributions due to the presence of highly heterogeneous county populations and extreme values. Log transformation reduces this skewness and compresses large values, potentially stabilizing gradients during training and improving the ability of the network to learn meaningful relationships between predictors and disaster assistance outcomes.

Comparing these two representations allows the experiments to assess whether distributional normalization of the SVI indicators improves predictive performance and model stability. The transformation choice is therefore treated as a preprocessing ablation rather than a fixed assumption.

### Network Capacity

Network capacity is evaluated by comparing the baseline architecture with deeper variants that increase both the width and depth of hidden layers. Increasing hidden-layer width allows the network to learn a richer set of nonlinear feature interactions, while increasing depth enables hierarchical feature transformations across multiple layers.

However, larger architectures may also increase the risk of overfitting, particularly when the number of counties is limited relative to the number of model parameters. Evaluating multiple architectures therefore helps determine whether additional capacity improves predictive accuracy or whether a smaller model provides a better balance between flexibility and generalization.

### Regularization

Additional experiments examine the effects of dropout and weight decay to identify a configuration that balances predictive accuracy and model stability. Dropout introduces stochastic noise during training by randomly deactivating hidden units, which can help prevent co-adaptation among neurons and improve generalization. Weight decay provides L2 regularization by penalizing large parameter values during optimization, encouraging smoother model solutions.

These regularization mechanisms are evaluated across a range of values to determine whether the model benefits more from explicit parameter constraints or stochastic regularization. The goal of this ablation stage is to identify a configuration that produces stable learning dynamics while maintaining strong predictive performance on the validation set.
...
The dataset was partitioned into train, validation, and test splits of 2,198, 471, and 472 counties respectively, with a batch size of 64 across both pipelines (@tbl-mlp-dataloader-summary). Both pipelines used the same baseline MLP architecture consisting of two hidden layers (64 → 32) with ReLU activations and light dropout regularization (@tbl-mlp-architecture). The model predicts a single continuous outcome: the log-transformed FEMA disaster assistance amount, $\log(1 + \text{FEMA IHP})$. This formulation treats the task as a regression problem, allowing the network to learn nonlinear relationships between the SVI indicators and observed disaster assistance outcomes.
...
The validation metrics support the use of the log-transformed feature pipeline. As shown in @tbl-mlp-val-metrics-overall, the model trained on log-transformed features achieved a lower RMSE (1.913993 vs. 2.043005) and higher R² (0.589123 vs. 0.531866) than the raw-feature pipeline. These results indicate modest but consistent performance improvements when using the transformed feature representation.
...
From a modeling perspective, these results suggest that county-level disaster assistance prediction benefits from moderately deep neural networks capable of capturing nonlinear relationships among vulnerability indicators. However, increasing model capacity beyond this point does not improve validation performance. The **Deep-128 architecture with log-transformed features** therefore provides the best balance between representational capacity and generalization.
...
The first experiment evaluated the effect of **dropout strength** within the Deep-128 network. Three dropout values were tested—**0.0, 0.1, and 0.3** while keeping all other training settings constant (@tbl-mlp-dropout-ablation-config). Each configuration was trained using both the raw and log-transformed pipelines.
...
After completing the architecture and regularization ablation experiments, the final MLP configuration selected for evaluation uses log-transformed features, the Deep-128 architecture ([128, 64, 32]), dropout = 0.1, and weight decay = 1e-5. As summarized in @tbl-mlp-best-config-summary, this configuration achieves the best validation performance among the tested models, with **validation RMSE = 1.9161** and **validation $R^2$ = 0.5882** when predicting log1p(FEMA IHP). These results indicate that moderate network capacity combined with light regularization provides the best balance between model flexibility and generalization for this task.
...
The ablation experiments identified a final MLP configuration using log-transformed features, the Deep-128 architecture ([128, 64, 32]), dropout = 0.1, and weight decay = 1e-5. This model achieved the best validation performance and was selected as the tabular benchmark for predicting log1p(FEMA IHP).

The final held-out evaluation shows moderate predictive performance, with **test RMSE = 2.0048** and **$R^2 = 0.5712$** (@tbl-final-mlp-test-evaluation). These results indicate that county-level SVI indicators explain a meaningful portion of the variation in FEMA disaster assistance, but they do not fully capture disaster outcomes. This is expected because assistance levels depend not only on social vulnerability but also on hazard exposure, disaster severity, and federal response dynamics.
...
joblib.dump(scaler_ylog, processed_dir / "scaler_ylog.pkl")
